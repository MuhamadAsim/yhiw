// components/provider/ChatPopup.tsx
...
interface ChatPopupProps {
  ...
  bookingId: string;
  customerName: string;
  customerId?: string; // Optional, if we have customer ID
}
    ...
    try {
      ...
      if (data.success && data.messages) {
        ...
        const formattedMessages: Message[] = data.messages.map((msg: any) => ({
          ...
          text: msg.text,
          sender: msg.sender === 'customer' ? 'customer' : 'provider',
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/chat/${bookingId}/send`, {
        ...
        body: JSON.stringify({
          text: messageText,
          receiverId: customerId || 'customer', // Use customer ID if available
        }),
      });
