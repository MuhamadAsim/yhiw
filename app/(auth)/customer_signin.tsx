...
const SignInScreen = () => {
  ...
  const checkIfCustomer = async (userEmail: string): Promise<boolean> => {
    try {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const isCustomer = await checkIfCustomer(email);

      if (!isCustomer) {
        // Sign out the user if they're not a customer

        Alert.alert(
          'Access Denied',
          'This account is registered as a service provider, not a customer. Please use the provider app to sign in.',
          [{ text: 'OK' }]
        );
        setIsLoading(false);
        return;
